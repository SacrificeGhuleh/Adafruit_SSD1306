...
Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, I2C *twi, int8_t rst_pin,
...
void Adafruit_SSD1306::ssd1306_command1(uint8_t c) {
    if (wire) { // I2C
        ...
        wire->write(i2caddr, cmd, sizeof(cmd));
...
//        }
    ...
    if (reset && (rstPin >= 0)) {
        DigitalOut resetPin(static_cast<PinName>(rstPin));
        //pinMode(rstPin, OUTPUT);
//        digitalWrite(rstPin, HIGH);
        resetPin.write(true);
        wait_ms(1);                   // VDD goes high at start, pause for 1 ms
...
void Adafruit_SSD1306::drawPixel(int16_t x, int16_t y, uint16_t color) {
    if ((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
        ...
        switch (color) {
            case WHITE: buffer[x + (y / 8) * WIDTH] |= (1 << (y & 7));
                break;
            case BLACK: buffer[x + (y / 8) * WIDTH] &= ~(1 << (y & 7));
                break;
            case INVERSE: buffer[x + (y / 8) * WIDTH] ^= (1 << (y & 7));
                break;
        }
    }
}
    ...
    if ((x >= 0) && (x < WIDTH)) { // X coord in bounds?
        ...
        if (__h > 0) { // Proceed only if height is now positive
            ...
            if (mod) {
                ...
                switch (color) {
                    case WHITE: *pBuf |= mask;
                        break;
                    case BLACK: *pBuf &= ~mask;
                        break;
                    case INVERSE: *pBuf ^= mask;
                        break;
                }
                pBuf += WIDTH;
            }
